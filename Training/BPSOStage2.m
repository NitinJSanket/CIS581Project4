%% Binary Particle Swarm Optimization Code for Face Detection Training Stage 2
...
function [gbest,FIT] = BPSOStage2(IG, NSub, NImag, NBlock, iterations, inertia, c1, c2, N, Window)
...
for i = 1:N                         % no of particles
    ...
    x1 = x(i,:);
    x1 = x1';
    fitpbest(i) = CalcFitnessStage2(x1,IG, NSub, NImag, NBlock); % store fitness values of all 30 pbests
    % CalcFitnessStage2(x,IG, NSub, NImag, NBlock)
...
for iter = 1:iterations
    for i=1:N
        x1 = x(i,:);
        x1 = x1';
        fitval = CalcFitnessStage2(x1,IG, NSub, NImag, NBlock);
        FIT(q) = fitval;
        q = q+1;
